...
export interface QueryEval {
  ...
  /**
   * Benchmark-only abstention-signal diagnostics. The
   * field is ADDITIVE: existing per-query eval fields and
   * report shapes are unchanged. The block is populated
   * only by the abstention-audit runner
   * (`runAbstentionAudit` in
   * `src/benchmark/abstention-audit-runner.ts`); the
   * regular single-variant / comparison / calibration
   * reports leave it `undefined` so a reviewer can
   * distinguish "the audit ran on this row" from "this
   * row was generated by the regular benchmark".
   *
   * The block carries the candidate abstention signals
   * the audit is studying. Each signal is a single
   * number; the audit applies them as abstain-if-not-
   * confident thresholds and measures how well they
   * separate answerable from no-answer queries.
   *
   * All signals are PURELY RETRIEVAL-DERIVED: no
   * answer-quality / LLM judging, no learned model. The
   * signals are documented in
   * `src/benchmark/abstention-audit.ts` (and surfaced
   * in the README's "How to interpret the audit"
   * section).
   */
  abstentionSignals?: Readonly<AbstentionSignals>;
}

/**
 * Per-query abstention-signal diagnostics. The
 * benchmark-only audit computes a fixed set of signals
 * for every query, then studies how well each signal
 * separates answerable from no-answer queries (AUROC),
 * and what the risk-coverage trade-off looks like under
 * the signal as a threshold gate.
 *
 * Every signal is documented on the field. All signals
 * are "higher = more confident" (so the audit can
 * compare them on a consistent scale). The audit
 * converts a signal into an abstention decision by
 * `abstain if signal < threshold`; the same direction
 * the existing `threshold@` / `margin@` / `ratio@`
 * gates use. A "perfect" signal would be high for every
 * answerable query and low for every no-answer query.
 */
export interface AbstentionSignals {
  // ----- Retrieval signals (top-1 score + per-candidate diagnostics) -----

  /** Top-1 score. Variant-specific scale (cosine / RRF / Jaccard). */
  topScore: number;
  /**
   * Top-1 to top-2 absolute score gap. 0 if fewer than 2
   * candidates. A small gap means the ranker is barely
   * confident the top-1 is better than the runner-up.
   */
  top1Top2Gap: number;
  /**
   * Top-1 / top-2 score ratio. `+Infinity` if top-1 > 0
   * and top-2 == 0 (only candidate); 1.0 if both == 0
   * (no signal). Mirrors the
   * `CalibrationQueryDiagnostic.scoreRatio` contract.
   */
  top1Top2Ratio: number;
  /** Number of candidates returned by the ranker. */
  returnedCount: number;

  // ----- Hybrid contributor signals (hybrid-dense only) -----

  /**
   * Number of contributors (out of the configured set)
   * that surfaced the top-1 candidate. For
   * `hybrid-dense` the contributor set is
   * `{lexical, fts5, vector-dense}`; the cap is 3. For
   * single-variant runs the value is 1 if the candidate
   * was returned, 0 if not. The value is `0` when the
   * ranker returned an empty top-K.
   */
  agreementCount: number;
  /**
   * Per-source minimum RRF rank across the contributors
   * that surfaced the top-1 candidate. `null` when the
   * top-1 candidate is absent from the contributor set;
   * `Number.POSITIVE_INFINITY` when the ranker returned
   * an empty top-K. The "best rank across contributors"
   * is a confidence signal: a rank-1 from any source
   * (value = 1) means at least one contributor strongly
   * surfaced the candidate.
   */
  minContributorRank: number | null;
  /** Per-source maximum RRF rank (parallel to `minContributorRank`). */
  maxContributorRank: number | null;
  /** Mean RRF rank across contributors. 0 when none. */
  meanContributorRank: number | null;
  /**
   * Per-source minimum raw score across contributors
   * that surfaced the top-1 candidate. `null` when the
   * top-1 candidate is absent from the contributor set.
   * The "worst-source score" is a signal: a low worst-
   * source score on a candidate means at least one
   * contributor was barely confident, even if another
   * contributor was strong.
   */
  minContributorScore: number | null;
  /** Per-source maximum raw score. */
  maxContributorScore: number | null;
  /** Mean raw score across contributors. 0 when none. */
  meanContributorScore: number | null;
  /**
   * Source presence pattern: a stable string of the form
   * `"L+_+V"` where each letter is `L` (lexical surfaced),
   * `F` (fts5 surfaced), `V` (vector(-dense) surfaced) and
   * `_` marks absence. For example `"L_V"` means lexical +
   * vector surfaced, fts5 absent. The "absent" pattern
   * `"___"` means no contributor surfaced the top-1. The
   * pattern is the single most direct hybrid-aware
   * confidence signal: a no-answer query that surfaces
   * with pattern `"LFV"` (all three contributors agreed)
   * is harder to detect than a no-answer query with
   * pattern `"_V_"` (only the vector surfaced).
   */
  ...
  /**
   * True iff the query matches the benchmark-only
   * "no-answer hard-negative" detector. The detector
   * looks for no-answer-family queries that share
   * tokens with real records (e.g. the labeled
   * `nonexistent-load-balancer` query, which shares
   * `MCP` / `server` / `port` tokens with the agent
   * runtime cluster). NOT the same as
   * `e.family === "no-answer"`: a no-answer query that
   * shares no tokens with the corpus is an "easy" no-
   * answer query, not a hard-negative.
   */
  isNoAnswerHardNegative: boolean;
  /**
   * True iff the query is a temporal-style "current" /
   * "now" / "today" query. The detector looks for words
   * like `current`, `now`, `today`, `currently`. A
   * reviewer should expect the ranker to surface the
   * current fact at the top; if the abstention signal
   * does not separate `isTemporalCurrent: true` from
   * `isTemporalCurrent: false`, the signal is missing
   * the temporal confidence dimension.
   */
  isTemporalCurrent: boolean;
  /**
   * True iff the query contains a negation-like token
   * (`no`, `not`, `never`, `without`, `none`,
   * `neither`, `nor`, `isn't`, `wasn't`, `aren't`,
   * `don't`, `doesn't`, `didn't`, `won't`, `wouldn't`).
   * Negation can flip the relevant fact (the user asked
   * what we DO NOT use, not what we use).
   */
  isNegationLike: boolean;
  /**
   * True iff the query mentions an entity that is
   * OOD-entity-like: shares tokens with the legacy /
   * previous cluster (records 21..24 and 93..96) and
   * is not in the expected-ids of any real record. The
   * detector is a soft heuristic; it is NOT keyed to
   * specific query ids.
   */
  isOodEntityLike: boolean;
  /**
   * True iff the query is a paraphrase-style trap: a
   * family === "paraphrase" query, or a query whose
   * token overlap with its expected id's summary is
   * low enough that the lexical baseline is expected
   * to miss. The detector uses the family label as
   * the primary signal; it does not key on specific
   * query ids.
   */
  isParaphraseTrap: boolean;
  /**
   * True iff the query is a false-premise-like
   * negative: a no-answer query that mentions an
   * entity / tool the corpus does not name (e.g. a
   * load-balancer, a CDN, an SSO). The detector uses
   * the family label and a small set of common
   * "tool" tokens; it is NOT keyed to specific
   * query ids.
   */
