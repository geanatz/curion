...
export function parseRetrievalCli(argv: string[]): RetrievalBenchmarkOptions {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--threshold" && argv[i + 1]) {
      ...
      // also support `--dense-skip` below. The composed
      // object also carries the `backend` selector so
      // the factory can route `--embedder qwen3
      // --dense-cache-dir /tmp/cache` to the Qwen3
      // dispatcher (the bare `--embedder qwen3` string
      // form already dispatches correctly; the object
      // form is needed when the cache dir / skip
      // overrides are also passed).
      //
      // The original spec string is preserved as
      // `baseObj.spec` so a string spec with
      // `key=value` keys (`qwen3:model=...,
      // dtype=...` or `embeddinggemma:model=...,
      // queryTask=...`) is not lost when the user
      // combines `--embedder <spec>` with
      // `--dense-cache-dir` / `--dense-skip`. The
      // factory's string-spec path is the
      // single source of truth for `key=value`
      // parsing; the object-spec path only honors
      // the well-known fields (`modelId`,
      // `dtype`, `cacheDir`, `skip`).
      ...
      if (typeof prev === "string") {
        if (
          ...
          prev === "embeddinggemma" ||
          prev.startsWith("embeddinggemma:") ||
          prev === "embedding-gemma" ||
          prev.startsWith("embedding-gemma:")
        ) {
          baseObj.backend = "embeddinggemma";
        } else if (
          prev === "bge-m3" ||
          prev.startsWith("bge-m3:") ||
          prev === "bgem3" ||
          prev.startsWith("bgem3:")
        ) {
          baseObj.backend = "bge-m3";
        } else if (prev === "stub-dense" || prev.startsWith("stub-dense:")) {
          baseObj.backend = "stub-dense";
        } else {
          baseObj.backend = "transformersjs";
        }
        // Preserve the original spec string so
        // the factory's string-spec dispatcher
...
function printRetrievalHelp(): void {
  process.stdout.write(
    [
      ...
      "                         'qwen3' (real local Qwen3-Embedding-0.6B ONNX via",
      "                            @huggingface/transformers 3.x; default model:",
      "                            onnx-community/Qwen3-Embedding-0.6B-ONNX, dtype=q8,",
      "                            pooling=last_token, normalize=true, dim=1024)",
      "                         'qwen3:model=<id>,dtype=<q8|q4|fp16|fp32>,task=<text>,pooling=<last_token|mean|cls|none>'",
      "                         'embeddinggemma' (real local EmbeddingGemma-300M ONNX via",
      "                            @huggingface/transformers 3.x; default model:",
      "                            onnx-community/embeddinggemma-300m-ONNX, dtype=q8,",
      "                            pooling=mean, normalize=true, dim=768;",
      "                            license=Gemma)",
      "                         'embeddinggemma:model=<id>,dtype=<q8|q4|fp16|fp32>,queryTask=<text>,pooling=<mean|last_token|cls|none>'",
      "                         'embedding-gemma' is an alias for 'embeddinggemma'",
      "                         'bge-m3' (real local BGE-M3 ONNX via",
      "                            @huggingface/transformers 3.x; default model:",
      "                            Xenova/bge-m3, dtype=q8,",
      "                            pooling=cls, normalize=true, dim=1024;",
      "                            license=MIT)",
  ...
  for (const q of queries) {
    ...
    if (variant === "vector-dense") {
      const dense = await rankDenseVectorWithMetadataAsync(q.query, candidates, {
        ...
        topK,
        embedder,
        // The benchmark runner embeds the user
        // query string. The `kind: "query"` flag
        // tells the Qwen3 embedder to apply the
        // `Instruct: <task>\nQuery:<query>`
        // instruction prefix on the query side
        // (and the unprefixed text on the
        // document / candidate side). The
        // fallback embedders (stub, MiniLM) ignore
        // the flag.
        kind: "query",
  ...
  for (const q of queries) {
    ...
    if (variant === "vector-dense") {
      const dense = await rankDenseVectorWithMetadataAsync(q.query, candidates, {
        ...
        // single-variant path uses; the Qwen3
        // embedder applies the
        // `Instruct: <task>\nQuery:<query>`
        // prefix on the query side and unprefixed
        // text on the document side.
        kind: "query",
      } satisfies DenseVectorRankingOptions);
      ranked = dense.hits;
    } else {
