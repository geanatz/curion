/**
 * Benchmark-only temporal ranking-preference
 * re-ranker diagnostic runner.
 *
 ...
 *      `topIds` / `topScores` / `currentTruthIds` /
 *      `passed` / `rank1` / `currentTruthAt1`).
 ...
 *      new dense benchmark.
 *   3. Calls `buildTemporalRerankReport` to
 *      produce the report.
 ...
 * `src/benchmark/temporal-ranking-preference.ts`.
 *
 * Determinism:
 ...
 *     `retrieval-baseline-*.json` artifact the
 *     re-ranker consumes. Default: the most
 *     recent `retrieval-baseline-*.json` under
 *     `.cortex/benchmark/`.
 ...
 * `TemporalRerankReport`. The function is pure: no
...
export function runTemporalRerankAnalysis(args: {
  ...
  return buildTemporalRerankReport({
...
const ARTIFACT_FILE_PREFIX = "retrieval-temporal-ranking-preference";

/**
 * Write a temporal re-ranker report to disk.
 * The file prefix is
 * `retrieval-temporal-ranking-preference-*.json`
 * (distinct from the existing audit / policy /
 * diagnostic / benchmark / calibration / no-
 * answer-abstention / false-abstention-damage /
 * paraphrase-recovery / temporal-truth-diagnostic
 * prefixes) so a reviewer can find the experiment
 ...
 * `.cortex/benchmark/`.
 */
export interface TemporalRerankCliArgs {
...
export async function runTemporalRerankCli(
  ...
  const report = runTemporalRerankAnalysis({
    benchmarkArtifact,
    ...(semantic ? { semantic } : {}),
...
const isMainEntry = (() => {
  ...
  if (!/temporal-ranking-preference-runner\.(ts|js)$/.test(entry)) return false;
  try {
...
// Re-export the import-only types so a downstream
// test can `import { type BenchmarkArtifact }` from
// this module. The re-export is a no-op at the
// type level; the runtime side is empty.
