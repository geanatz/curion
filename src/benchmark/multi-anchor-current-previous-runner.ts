 ...
 *      new dense benchmark.
 *   3. Calls `buildMultiAnchorRerankReport`
 ...
 * `src/benchmark/multi-anchor-current-previous.ts`.
 *
 * Determinism:
 ...
 * map and emits the
 * `MultiAnchorRerankReport`. The function is
 * pure: no I/O. The CLI entry point writes the
 ...
 */
export function runMultiAnchorRerankAnalysis(args: {
  ...
  return buildMultiAnchorRerankReport({
...
const ARTIFACT_FILE_PREFIX = "retrieval-multi-anchor-current-previous";

/**
 * Write a multi-anchor re-ranker report to disk.
 * The file prefix is
 ...
 * supersession-edge-simulation prefixes) so a
 ...
 * `.curion/benchmark/`.
 */
export interface MultiAnchorRerankCliArgs {
...
export async function runMultiAnchorRerankCli(
  ...
  const report = runMultiAnchorRerankAnalysis({
    benchmarkArtifact,
    ...(semantic ? { semantic } : {}),
  ...
  if (!args.noWrite) {
    written = writeMultiAnchorRerankReport(report, outDir);
  }
  if (!args.noStdout) {
