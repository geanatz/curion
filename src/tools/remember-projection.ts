 ...
 * `summary`, `safetyClass`, `question`, etc.). The MCP `text`
 * content block and the MCP `structuredContent` are derived
 * from that result by this module. The projection is the
 * single place where the user-approved wire shape is
 * enforced:
 *
 ...
 *     (`rejected`, `clarification_needed`, `provider_error`)
 *     is the existing `RememberResult.message` form.
 ...
 * the literal `question` text, none of which contain a
 * `Note:` prefix.
 */
 ...
 * `RememberResult.message` for all four statuses. The
 * function exists to keep the wire-projection interface
 ...
 *   - `clarification_needed`-> { status, question }
...
export function buildRememberStructuredContent(
  ...
  switch (result.status) {
    ...
    case "clarification_needed": {
      // The public `message` is `outcome.question` (the
      // `formatOutcome` for this case sets `message` to
      // `outcome.question` and also populates the
      // structured `question` field). Surface the same
      // string as `question` for the structuredContent
      // so clients can read it directly without parsing
      // the `message` field.
      const question = result.question ?? result.message;
