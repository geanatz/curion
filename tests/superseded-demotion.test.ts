...
function mkCandidate(
  ...
  return relationship === undefined ? { id, score } : { id, score, relationship };
}
...
test("demoteSupersededMemories: A supersedes B -> B is demoted below A", () => {
  ...
  const candidates = [
    mkCandidate(5, 0.8, { supersededBy: [10] }), // B is stale
    mkCandidate(10, 0.75, { supersedes: [5] }), // A is current
    mkCandidate(20, 0.7), // unrelated C
  ];
  ...
  assert.equal(result[2]!.score, 0.8 * DEMOTION_FACTOR);
});
...
test("demoteSupersededMemories: supersededBy reference -> stale candidate demoted", () => {
  ...
  const candidates = [
    mkCandidate(10, 0.9, { supersedes: [5] }), // A is current
    mkCandidate(5, 0.85, { supersededBy: [10] }), // B is stale
    mkCandidate(15, 0.8), // unrelated C
  ];
  const result = demoteSupersededMemories(candidates);
...
test("demoteSupersededMemories: unrelated candidates preserve relative order", () => {
  ...
  const candidates = [mkCandidate(1, 0.8), mkCandidate(2, 0.7), mkCandidate(3, 0.6)];
  const result = demoteSupersededMemories(candidates);
  assert.deepEqual(
    result.map((r) => r.id),
    [1, 2, 3]
...
test("demoteSupersededMemories: no relationship blocks -> no demotion", () => {
  ...
  const candidates = [mkCandidate(100, 0.9), mkCandidate(50, 0.8), mkCandidate(25, 0.7)];
  const result = demoteSupersededMemories(candidates);
  assert.deepEqual(
    result.map((r) => r.id),
    [100, 50, 25]
...
test("demoteSupersededMemories: supersedes target not in candidate list -> no demotion", () => {
  ...
  // No demotion should occur.
  const candidates = [
    mkCandidate(10, 0.8, { supersedes: [5, 99] }), // A supersedes B(5) and C(99), neither present
    mkCandidate(20, 0.7),
  ];
  const result = demoteSupersededMemories(candidates);
  // No demotion; scores unchanged, order by score desc then id desc.
  assert.deepEqual(
    result.map((r) => ({ id: r.id, score: r.score })),
    [
      { id: 10, score: 0.8 },
      { id: 20, score: 0.7 },
    ]
...
test("demoteSupersededMemories: supersededBy referrer not in candidate list -> no demotion", () => {
  ...
  const candidates = [
    mkCandidate(5, 0.8, { supersededBy: [10, 20] }), // B stale, but referrers not present
    mkCandidate(30, 0.7),
  ];
  const result = demoteSupersededMemories(candidates);
...
test("demoteSupersededMemories: A supersedes B, B supersededBy C -> B and A are not staleness partners", () => {
  ...
  const candidates = [
    mkCandidate(5, 0.8, { supersededBy: [10, 20] }), // B is stale
    mkCandidate(10, 0.75, { supersedes: [5] }), // A is current
    mkCandidate(20, 0.7, { supersedes: [5] }), // C is also current
  ];
  const result = demoteSupersededMemories(candidates);
...
test("demoteSupersededMemories: two independent supersession pairs handled", () => {
  ...
  const candidates = [
    mkCandidate(5, 0.9, { supersededBy: [10] }), // B1 stale
    mkCandidate(10, 0.85, { supersedes: [5] }), // A1 current
    mkCandidate(15, 0.8, { supersededBy: [20] }), // B2 stale
...
test("demoteSupersededMemories: chain supersession A->B->C", () => {
  ...
  // A is current.
  const candidates = [
    mkCandidate(5, 0.7, { supersededBy: [10] }), // C is stale
    mkCandidate(10, 0.8, { supersedes: [5], supersededBy: [15] }), // B is stale (supersededBy A)
    mkCandidate(15, 0.9, { supersedes: [10] }), // A is current
  ];
  const result = demoteSupersededMemories(candidates);
...
test("demoteSupersededMemories: does not mutate the input array", () => {
  const candidates = [
    mkCandidate(5, 0.8, { supersededBy: [10] }),
...
test("demoteSupersededMemories: returns a new array", () => {
  const candidates = [mkCandidate(1, 0.8)];
  const result = demoteSupersededMemories(candidates);
...
test("demoteSupersededMemories: single candidate -> no demotion", () => {
  const candidates = [mkCandidate(5, 0.8, { supersededBy: [10] })];
  const result = demoteSupersededMemories(candidates);
...
test("demoteSupersededMemories: two candidates but no supersession between them", () => {
  const candidates = [mkCandidate(1, 0.8), mkCandidate(2, 0.7)];
  const result = demoteSupersededMemories(candidates);
  assert.deepEqual(
    result.map((r) => r.id),
    [1, 2]
...
test("demoteSupersededMemories: undefined relationship -> treated as no relationship", () => {
  const candidates = [
    { id: 5, score: 0.8 }, // no relationship
    { id: 10, score: 0.75, relationship: undefined }, // explicitly undefined
    { id: 20, score: 0.7, relationship: { supersedes: [5] } }, // A supersedes B
  ];
...
test("demoteSupersededMemories: demoted candidate loses tie to non-stale candidate", () => {
  ...
  const candidates = [
    mkCandidate(5, 0.8, { supersededBy: [10] }), // B stale
    mkCandidate(10, 0.75, { supersedes: [5] }), // A current
...
test("demoteSupersededMemories: multiple stale candidates sort by demoted score", () => {
  ...
  const candidates = [
    ...
    mkCandidate(15, 0.9, { supersededBy: [20] }), // B2 stale
    mkCandidate(10, 0.75, { supersedes: [5] }), // A1 current
    mkCandidate(20, 0.7, { supersedes: [15] }), // A2 current
  ];
  const result = demoteSupersededMemories(candidates);
...
test("demoteSupersededMemories: self-supersession (A supersedes A) -> ignored, no crash", () => {
  ...
  const candidates = [
    mkCandidate(10, 0.8, { supersedes: [10] }), // self-ref
  ];
  // Must not throw.
  const result = demoteSupersededMemories(candidates);
  assert.equal(result.length, 1);
  assert.equal(result[0]!.id, 10);
  assert.equal(result[0]!.score, 0.8); // unchanged
});
...
test("demoteSupersededMemories: self-supersededBy (A supersededBy A) -> ignored, no crash", () => {
  ...
  const candidates = [
    mkCandidate(10, 0.8, { supersededBy: [10] }), // self-ref
  ];
  const result = demoteSupersededMemories(candidates);
  assert.equal(result.length, 1);
  assert.equal(result[0]!.id, 10);
...
test("demoteSupersededMemories: self-supersession with other valid supersession", () => {
  ...
  const candidates = [
    mkCandidate(5, 0.8, { supersededBy: [10] }), // B is superseded by A
  ...
  assert.equal(result[0]!.score, 0.8 * DEMOTION_FACTOR);
...
test("demoteSupersededMemories: duplicate supersedes entries -> B demoted once", () => {
  ...
  assert.equal(result[1]!.score, 0.8 * DEMOTION_FACTOR);
});
...
test("demoteSupersededMemories: duplicate supersededBy entries -> B demoted once", () => {
  ...
  const candidates = [
    mkCandidate(5, 0.8, { supersededBy: [10, 10] }), // B stale (duplicate)
    mkCandidate(10, 0.75, { supersedes: [5] }), // A current
  ];
  const result = demoteSupersededMemories(candidates);
  assert.equal(result[0]!.id, 10); // A first
...
test("demoteSupersededMemories: both supersedes and supersededBy duplicate -> correct demotion", () => {
  ...
  const candidates = [
    mkCandidate(5, 0.9, { supersededBy: [10, 10] }), // B stale (duplicate)
    mkCandidate(10, 0.75, { supersedes: [5, 5] }), // A supersedes B (duplicate)
  ];
  const result = demoteSupersededMemories(candidates);
  assert.equal(result[0]!.id, 10); // A first
  assert.equal(result[1]!.id, 5); // B demoted
  assert.equal(result[1]!.score, 0.9 * DEMOTION_FACTOR);
});
...
test("demoteSupersededMemories: supersedes with non-finite number -> safely ignored", () => {
  ...
  const candidates = [
    mkCandidate(5, 0.8, { supersededBy: [10] }),
    mkCandidate(10, 0.75, {
      supersedes: [5, Number.NaN, Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY],
    }),
  ];
  const result = demoteSupersededMemories(candidates);
  assert.equal(result[0]!.id, 10);
...
test("demoteSupersededMemories: supersededBy with non-finite numbers only -> no demotion", () => {
  ...
  const candidates = [
    mkCandidate(5, 0.8, {
      supersededBy: [Number.NaN, Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY],
    }),
    mkCandidate(10, 0.75),
  ];
  const result = demoteSupersededMemories(candidates);
...
test("demoteSupersededMemories: empty supersededBy array -> no action", () => {
  const candidates = [
    mkCandidate(5, 0.8, { supersededBy: [] }),
    mkCandidate(10, 0.75, { supersedes: [5] }),
  ];
  const result = demoteSupersededMemories(candidates);
...
test("demoteSupersededMemories: unrelated candidates with ties broken by id desc", () => {
  ...
  const candidates = [mkCandidate(1, 0.8), mkCandidate(2, 0.8), mkCandidate(3, 0.8)];
  const result = demoteSupersededMemories(candidates);
  // id desc -> 3, 2, 1
  assert.deepEqual(
    result.map((r) => r.id),
