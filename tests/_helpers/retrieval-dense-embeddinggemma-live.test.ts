 ...
 * EmbeddingGemma dense retrieval benchmark.
 *
 * This test exercises the actual
 * `@huggingface/transformers` integration: it
 * constructs an `EmbeddingGemmaEmbedder` with
 * the default pinned model, calls `init()`,
 * and verifies the embedder actually produces
 * a real semantic vector (a semantically-
 * similar pair scores higher than an unrelated
 * pair).
 *
 * It is the EmbeddingGemma counterpart to the
 * existing
 * `tests/_helpers/retrieval-dense-qwen3-live.test.ts`
 * (which exercises the Qwen3 path) and the
 ...
 *     first run (~309MB for the q8 ONNX,
 *     smaller than the Qwen3 ~600MB but still
 *     a real network + disk cost). A CI run
 *     that already has the model in the local
 *     cache is fast, but a fresh run is a real
 *     network + disk cost.
 *   - The test is opt-in: it is NOT in the
 *     default `npm test` glob
 *     (`tests/*.test.ts`) and is NOT in
 *     `tests/_helpers/`. The runner that wants
 ...
 *     at test time. If the cache is empty, the
 *     `init()` call will attempt a download;
 *     the test reports `skip` on a clear
 *     network or library-missing error and
 *     `pass` when the model is in the local
 *     cache.
 *
 * The test is intentionally minimal so it can
 * be added to a future "live integration" CI
 * job without adding to the unit-test
 * surface.
 *
 * Usage:
...
/**
 * Probe whether `@huggingface/transformers`
 * is importable in this environment. The
 * package is declared as a devDependency but
 * may not be installed in the CI sandbox; the
 * test is gracefully skipped in that case. The
 * probe uses a dynamic import inside an async
 * function so a missing module is caught as a
 * rejection, not a synchronous syntax error.
 */
...
test("EmbeddingGemmaEmbedder live: init() produces a real semantic embedding with the documented prompt templates", async (t) => {
  ...
  try {
    ...
    // skip the test gracefully. The
    // EmbeddingGemma embedder's `init()`
    // typically catches the error internally
    // and flips the metadata to
    // `status: "error"`; the throw path is a
    // belt-and-braces fallback.
    const msg = err instanceof Error ? err.message : String(err);
    t.skip(`EmbeddingGemma init() failed (network or library missing): ${msg}`);
    return;
  }
  // The EmbeddingGemma embedder's `init()`
  // catches errors internally and flips the
  // metadata to `status: "error"`. A
  // network-less environment (no model in
  // the local cache, no network) lands here
  // with `status: "error"`. The test skips
  // gracefully in that case so a CI
  // environment without network can still
  ...
  // higher. The EmbeddingGemma model expects
  // the `task: ... | query: ...` prefix on
  // the query side and the `title: none |
  ...
  // DIFFERENT embeddings under EmbeddingGemma
  // (the prompt templates on the query side
  // and the document side change the model's
  // forward pass). The assertion is the live
  // contract: a real EmbeddingGemma model
  // applies the prefixes.
