/**
 * Live real-model integration test for the
 * BGE-M3 dense retrieval benchmark.
 *
 * This test exercises the actual
 * `@huggingface/transformers` integration: it
 * constructs a `BgeM3Embedder` with the
 * default pinned model, calls `init()`, and
 * verifies the embedder actually produces a
 * real semantic vector (a semantically-
 * similar pair scores higher than an
 * unrelated pair).
 *
 * It is the BGE-M3 counterpart to the
 * existing
 ...
 *   - The model download is a one-time cost
 *     on first run (~568MB for the q8 ONNX;
 *     the BGE-M3 dense mode artifact is the
 *     largest of the three local-model
 *     candidates). A CI run that already
 *     has the model in the local cache is
 *     fast, but a fresh run is a real network
 *     + disk cost.
 *   - The test is opt-in: it is NOT in the
 *     default `npm test` glob
 *     (`tests/*.test.ts`). The runner that
 *     wants this test must invoke it
 *     explicitly (e.g. `node --import tsx
 *     --test
 *     tests/_helpers/retrieval-dense-bge-m3-live.test.ts`).
 *   - The test does NOT depend on the
 *     network at test time. If the cache is
 *     empty, the `init()` call will attempt a
 *     download; the test reports `skip` on a
 *     clear network or library-missing error
 *     and `pass` when the model is in the
 *     local cache.
 *
 * The test is intentionally minimal so it
 * can be added to a future "live integration"
 * CI job without adding to the unit-test
 * surface.
 *
 * Usage:
...
function makeCacheDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "curion-bge-m3-live-cache-"));
}

/**
 * Probe whether
 * `@huggingface/transformers` is importable
 * in this environment. The package is
 ...
 * uses a dynamic import inside an async
 * function so a missing module is caught as
 * a rejection, not a synchronous syntax
 * error.
 */
...
test("BgeM3Embedder live: init() produces a real semantic embedding with the kind-agnostic contract", async (t) => {
  ...
  t.after(() => {
    try {
      ...
      // Best effort; the test's assertions
  ...
  try {
    ...
    // skip the test gracefully. The BGE-M3
    // embedder's `init()` typically catches
    // the error internally and flips the
    // metadata to `status: "error"`; the
    // throw path is a belt-and-braces
    // fallback.
    const msg = err instanceof Error ? err.message : String(err);
    t.skip(`BGE-M3 init() failed (network or library missing): ${msg}`);
    return;
  }
  // The BGE-M3 embedder's `init()` catches
  // errors internally and flips the metadata
  // to `status: "error"`. A network-less
  // environment (no model in the local cache,
  // no network) lands here with
  // `status: "error"`. The test skips
  // gracefully in that case so a CI
  ...
  if (meta.status !== "ready") {
    ...
    t.skip(`BGE-M3 init() did not reach ready (network or model missing): ${errMsg}`);
    return;
  }
  assert.equal(meta.backend, "bge-m3");
  assert.equal(meta.modelId, "Xenova/bge-m3");
  // The conventional BGE-M3 dense dim is
  // 1024. The `init()` probe may overwrite
  ...
  // higher. The BGE-M3 dense mode is
  // kind-agnostic: queries and documents are
  // forwarded verbatim (no instruction
  // prefix).
  ...
  // unit length (the BGE-M3 backend applies
  // `normalize: true`).
  ...
  // BGE-M3 dense mode is kind-agnostic: a
  // query and a document with the SAME plain
  // text produce IDENTICAL embeddings
  // (because the BGE-M3 dense mode applies
  // no instruction prefix on either side).
  // The assertion is the live contract: a
  // real BGE-M3 model has `embedQuery` and
  // `embedDocument` as plain forwarders.
  ...
  assert.equal(
    equal,
    true,
    "BGE-M3 live: embedQuery and embedDocument must produce identical vectors for the same input text (kind-agnostic dense mode)"
  );
});
