...
test("createDenseEmbedder: skip flag returns the stub without invoking init", async () => {
  const { embedder, spec } = await createDenseEmbedder("transformersjs", {
    skip: true,
  });
  // The factory did NOT init() the embedder, so the
  // factory falls back to the deterministic stub. The
  // benchmark runner treats `skip: true` as "do not
  // invoke the real embedder" and the runner uses the
  // stub for every embed call. The metadata is
  // `status: "ready"` (the stub is always ready), not
  // `status: "skipped"` (which would mean the
  // transformers.js embedder never ran). The `spec`
  // returned to the caller records the skip so a
  // reviewer can audit it.
  assert.equal(embedder.metadata.backend, "stub-dense");
  assert.equal(embedder.metadata.status, "ready");
  assert.equal(spec, "stub-dense (skipped)");
});
